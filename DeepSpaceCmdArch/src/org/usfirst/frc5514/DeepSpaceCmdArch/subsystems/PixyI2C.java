...
public class PixyI2C {
	...
	public PixyI2C(String id, 
				I2C argPixy, 
				PixyPacket[] argPixyPacket, 
				PixyException argPixyException, 
	...
	public PixyPacket readPacket(int Signature) throws PixyException {
		...
		for (int i = 0; i <= 16; i++) {
			...
			// Check if first 2 byes equal a "sync word", which indicates the
			// start of a packet of valid data
			if (syncWord == 0xaa55) { 
				
				// Parse the next 2 bytes
				syncWord = cvt(rawData[i + 3], rawData[i + 2]); 
				
				// Sometimes only a single sync word exists, if so, shift data over
				if (syncWord != 0xaa55) { 
					i -= 2;
				}
